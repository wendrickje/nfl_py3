...
## Splashsports pool workflow

Scoring a weekly ATS pool card at Splashsports' own team-pickem lines --
rather than the sharp market's opener -- needs none of `weekly-run`'s
publish path (no active-model activation, no `opener-evaluation`, no
`THE_ODDS_API_KEY`). It needs a feature table and a lines file.

On an empty clone, bootstrap the four raw snapshots `weekly-run` itself only
*refreshes* (it has nothing to refresh from on a fresh checkout):

```powershell
.\.tools\uv.exe run python scripts\bootstrap_ingest.py
```

This runs `ingest`, `pbp-ingest`, `player-ingest`, and `player-value-ingest`
in sequence, capping the three outcome ingests at the last *completed*
season by default (games not yet played have no play-by-play, injury, snap,
or production data to fetch). It deliberately skips `odds-backfill`: that
only feeds `opener-evaluation`'s cosmetic historical-accuracy headline on a
*published* card, which this workflow never builds, and it spends paid Odds
API credits for no benefit here. Then build the feature table the model
scores against, e.g. `nfl-ats build-features` (add `build-pbp-features` /
`build-player-features` for a richer profile -- see Commands below).

Signing in to Splashsports once, in a real (non-headless) browser window --
Splashsports' bot-detection blocks headless Chromium outright, even with a
valid saved session:

```powershell
.\.tools\uv.exe run nfl-ats splashsports-login
```

This opens a browser for you to log in by hand and saves the session to
`data/market/splashsports/auth_state.json` (gitignored). Every subsequent
week, archive that week's pool spreads with it, still non-headless:

```powershell
.\.tools\uv.exe run nfl-ats splashsports-ingest --headed
```

Convert that snapshot into a `pool-card-at-lines` lines file. Splashsports
reports each team's own spread in ordinary sportsbook notation (favorite
negative); this repo's `home_spread` convention is the opposite (positive
favors home, matching `nfl_ats.market_data`'s Odds API sign flip and
`spread_line`'s own convention elsewhere in the codebase) -- the script
handles that conversion, the team-nickname-to-abbreviation join against the
schedule, and preserves Splashsports' own picksheet order (not kickoff
time, since that is the order picks are compared against on the pool site):

```powershell
.\.tools\uv.exe run python scripts\splashsports_lines_file.py --season 2026 --week 1
```

Score the week at those lines with whatever feature profile/method/regressor
you built (match `artifacts/active_ats_model.json` if you want the same
configuration as the published card):

```powershell
.\.tools\uv.exe run nfl-ats pool-card-at-lines --season 2026 --week 1 `
  --features data\processed\game_features_player.parquet --feature-profile player `
  --method market_residual --regressor ridge --ridge-alpha 10.0 `
  --lines-file data\market\splashsports\lines_2026_week1.csv
```

The resulting `pool_card.md`/`pool_card.csv` under
`artifacts/pool_at_lines/<season>-week-<week>-<method>-<run_id>/` gives one
forced pick per game with the model's own probability at Splashsports'
actual number -- not historical accuracy, and not proof of a profitable or
stable edge (see AGENTS.md).
...
`splashsports-login`, `splashsports-ingest` (Splashsports team-pickem pool
spreads, session-based login -- see Splashsports pool workflow above).
